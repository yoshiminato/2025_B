...
export const generateBase64Image = onRequest(async (request, response) => {
  // Firebase Loggerでログ出力
  logger.info("generateBase64Image 関数が呼び出されました", {
    method: request.method,
    url: request.url,
    headers: request.headers,
  });
  ...
  try {
    ...
    for (const part of result.candidates[0].content.parts) {
      ...
      // テキストパートのログ出力(特に意味はない)
      if(part.text) {
        logger.info("テキストデータ:", part.text);
      }
      
      // 画像データの抽出
      if(part.inlineData){
        // inlineDataが存在する場合はBase64エンコードされた画像データを取得
        base64Image = part.inlineData.data;
      }
    }

    // 画像データが見つからない場合のエラーハンドリング
    if (!base64Image) {
      logger.error("画像データが見つかりませんでした");
      response.status(500).json({ error: '画像データが見つかりませんでした' });
      return;
    }

    // 正常に画像データが取得できた場合はimageDataとして返す
    response.status(200).json({ imageData: base64Image });
